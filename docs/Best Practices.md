...
###Error
`logger.log_error` should be used when a PL/SQL error has occurred. In most cases this is in an exception block. Regardless of any other configuration, `log_error` will store the callstack, along with the `CALL_ID`/`CALL_DEPTH`/`ROOT_UNIT_NAME` call hierarchy columns (see the `INS_LOGGER_LOGS` section of the [Logger API](Logger%20API.md) doc) - both are always populated for `log_error`, independent of the `INCLUDE_CALL_STACK` preference. Errors are considered actionalble items as an error has occurred and something (code, configuration, server down, etc) needs attention.

###Grouping related log entries
When `INCLUDE_CALL_STACK` is enabled (the default), every log call is automatically tagged with `CALL_ID`, `CALL_DEPTH`, and `ROOT_UNIT_NAME`. `CALL_ID` is shared by every log row that belongs to the same logical call chain, so `select * from logger_logs where call_id = '...' order by id` reconstructs the full set of log entries produced by one top-level operation, in nesting order - no code changes or manual scope conventions required. `LOGGER_LOGS_5_MIN` and `LOGGER_LOGS_60_MIN` add a `CALL_TREE` column that indents `UNIT_NAME` based on `CALL_DEPTH` and order their rows by `call_id, id`, so the nesting is visible at a glance without needing to filter by a specific `CALL_ID` first.
